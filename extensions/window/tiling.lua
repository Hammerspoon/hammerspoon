...
local function getCentroid(switchedRectToFill,windowArea,i,n,fillVertically)
  ...
  if not fillVertically then r=switchedwh(r) r:move(r.w/n*(i-1),0):setw(r.w/n)
  else r:move(0,r.h/n*(i-1)):seth(r.h/n) end
  return r.center
end

--- hs.window.tiling.tileWindows(windows,rect[,desiredAspect[,preserveRelativeArea[,animationDuration]]])
--- Function
...
---
--- Notes:
---   * The tiling engine will try to maintain the spatial distribution of windows, i.e. (roughly speaking) pick the closest window for each
---     "tile"; in some cases this isn't possible and the windows might get "reshuffled" around in unexpected ways
...
local function tileWindows(windows,rect,desiredAspect,preserveRelativeArea,animationDuration)
  ...
  elseif type(desiredAspect)~='number' then
    desiredAspect=geom.new(desiredAspect).aspect
  end
  ...
  for _,w in ipairs(windows) do
    ...
    wins[{frame=f,area=f.area,window=w}]=true
  end
  ...
  for win in pairs(wins) do
    ...
    log.vf('window at %.2f,%.2f, area %.2f',win.posInUnion.x,win.posInUnion.y,win.area)
  end
  ...
  repeat
    ...
    for dir=1,2 do
      ...
      local rowAspect=fillVertically and desiredAspect or 1/desiredAspect
      ...
      repeat
        ...
        for i=1,testn do
          ...
          local win=closestWindow(windowsRemaining,centroid)
          log.vf('next centroid at %.2f,%.2f, pick window at %.2f,%.2f (distance %.2f)',centroid.x,centroid.y,win.posInUnion.x,win.posInUnion.y,win.posInUnion:distance(centroid))
          testWindows[i]=win
        ...
        for _,win in ipairs(testWindows) do
          ...
          local winAspectRatio=(testFrame.w/testFrame.h)/rowAspect
          ...
          testFrame.y=testFrame.y+testFrame.h
        end
        rowAspectRatio[dir]=max(rowAspectRatio[dir] or testAspectRatio,testAspectRatio)
        if not next(windowsRemaining) and testAspectRatio<rowAspectRatio[dir] then
          --if this is the last window, consider what would happen if it's refused here and goes into its own row
          ...
          local lastAspectRatio=lastAspect/rowAspect
      ...
      log.vf('made a %s with %d windows, optimality %.0f%%',fillVertically and 'column' or 'row',tlen(rowWindows[dir]),rowAspectRatio[dir]*100)
    end
