...
local function windowEvent(win,event,_,appname)
  ...
  if not id and app then
    for _,awindow in pairs(app.windows) do
      if awindow.window==win then id=awindow.id break end
    end
  end
  ...
  local bwindow = app.windows[id]
  if not bwindow then return log.ef('%s (&d) is not registered!',appname,id) end
...
function WF:subscribe(event,fn,immediate)
  ...
  if type(k)=='string' then
    if type(v)=='function' then for ev,afn in pairs(event) do map[ev]={afn} end
  ...
  if immediate then
    ...
    for ev,fns in pairs(map) do
      for _,win in ipairs(windows) do
        ...
        then for _,afn in ipairs(fns) do
          afn(win.window,win.app.name,ev) end
        end
      end
      local win=windows[1]
      if win then
        if ev==windowfilter.hasWindow then
          for _,afn in ipairs(fns) do afn(win.window,win.app.name,ev) end
        end
...
function WF:unsubscribe(theEvents,fns)
  if not theEvents and not fns then error('you must pass at least one of event or fn',2) end
  local tevents,tfns=type(theEvents),type(fns)
  if theEvents==nil then tevents=nil end
  if fns==nil then tfns=nil end
  if tfns=='function' then fns={fns} tfns='lfn' end --?+fn
  if tevents=='function' then fns={theEvents} tfns='lfn' tevents=nil --omitted+fn
  elseif tevents=='string' then events={theEvents} tevents='ls' end --event+?
  if tevents=='table' then
    local k,v=next(theEvents)
    if type(k)=='function' and v==true then fns=theEvents tfns='sfn' tevents=nil --omitted+set of fns
    ...
    elseif type(k)=='number' then --list of events or functions
      if type(v)=='function' then fns=theEvents tfns='lfn' tevents=nil --omitted+list of fns
  ...
  if tevents==nil then theEvents=self.events tevents='ss' end --all events
  if tevents=='ss' then local l={} for k in pairs(theEvents) do l[#l+1]=k end theEvents=l tevents='ls'  end --make list
  if tfns=='sfn' then local l={} for k in pairs(fns) do l[#l+1]=k end fns=l tfns='lfn' end --make list

  if tevents=='map' then for ev,fn in pairs(theEvents) do unsubscribe(self,ev,fn) end
elseif tevents=='mapl' then for ev,afns in pairs(theEvents) do for _,fn in ipairs(afns) do unsubscribe (self,ev,fn) end end
  else
    if tevents~='ls' then error('invalid event parameter',2)
    elseif tfns~=nil and tfns~='lfn' then error('invalid fn parameter',2) end
    for _,ev in ipairs(theEvents) do
