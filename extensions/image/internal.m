...
NSColor *getColorFromStack(lua_State *L, int idx) {
    ...
    switch (lua_type(L, idx)) {
        ...
        case LUA_TNONE:
            ...
            break;
        default:
            CLS_NSLOG(@"ERROR: Unexpected type passed to an hs.image color method: %d", lua_type(L, idx));
            showError(L, (char *)[[NSString stringWithFormat:@"Unexpected type passed to an hs.image color method: %s", lua_typename(L, lua_type(L, idx))] UTF8String]) ;
            return nil;
