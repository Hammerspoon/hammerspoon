...
+ (NSData *_Nullable)dataWithEnvelope:(SentryEnvelope *)envelope
    ...
    for (int i = 0; i < envelope.items.count; ++i) {
        ...
        if (nil != envelope.items[i].header) {
            ...
            [serializedData setValue:[NSNumber numberWithUnsignedInteger:envelope.items[i].header.length] forKey:@"length"];
        }
...
+ (SentryEnvelope *_Nullable)envelopeWithData:(NSData *)data {
    ...
    for (int i = 0; i < data.length; ++i) {
        if (bytes[i] == '\n') {
            ...
            [SentryLog logWithMessage:[NSString stringWithFormat:@"Header %@", headerString] andLevel:kSentryLogLevelDebug];
#endif
            NSError *error = nil;
            NSDictionary *headerDictionary = [NSJSONSerialization JSONObjectWithData:headerData options:0 error:&error];
            if (nil != error) {
                [SentryLog logWithMessage:[NSString stringWithFormat:@"Failed to parse envelope header %@", error] andLevel:kSentryLogLevelError];
            } else {
    ...
    if (nil == envelopeHeader) {
        [SentryLog logWithMessage:[NSString stringWithFormat:@"Invalid envelope. No header found."] andLevel:kSentryLogLevelError];
        return nil;
    }
    ...
    for (NSInteger i = itemHeaderStart; i <= endOfEnvelope; ++i) {
        if (bytes[i] == '\n' || i == endOfEnvelope) {
            ...
            NSData *itemHeaderData = [data subdataWithRange:NSMakeRange(itemHeaderStart, i - itemHeaderStart)];
#ifdef DEBUG
            NSString *itemHeaderString = [[NSString alloc] initWithData:itemHeaderData encoding:NSUTF8StringEncoding];
            [SentryLog logWithMessage:[NSString stringWithFormat:@"Item Header %@", itemHeaderString] andLevel:kSentryLogLevelDebug];
#endif
            NSError *error = nil;
            NSDictionary *headerDictionary = [NSJSONSerialization JSONObjectWithData:itemHeaderData options:0 error:&error];
            if (nil != error) {
                [SentryLog logWithMessage:[NSString stringWithFormat:@"Failed to parse envelope item header %@", error] andLevel:kSentryLogLevelError];
                return nil;
            }
            NSString *_Nullable type = [headerDictionary valueForKey:@"type"];
            if (nil == type) {
                [SentryLog logWithMessage:[NSString
                                stringWithFormat:@"Envelope item type is required."]
