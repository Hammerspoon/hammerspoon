...
---
--- hs.grille uses hs.winter and one can use all the commands that hs.winter supports.
---
...
local function grille_focused(self)
  return winter.focused(self.ct)
end

local function grille_window(self, title)
  return winter.window(title or '', self.ct)
end

--- hs.grille.new(width, height) -> winter object
--- Function
--- Creates a new winter object that uses coordinate tranformations to place the windows on a grid with given width and height. Default width and height are 3.
---
--- Parameters:
...
---
--- Notes:
---  * See the documentation for winter module
