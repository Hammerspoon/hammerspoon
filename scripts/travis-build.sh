...
if [ "${OP}" == "build" ]; then
    ...
    ASAN_LIB_PATH=$(find /Applications/Xcode-beta.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/lib/clang/ -name libclang_rt.asan_osx_dynamic.dylib)
    if [ "${ASAN_LIB_PATH}" == "" ]; then
        echo "No beta Xcode found, trying non-beta..."
