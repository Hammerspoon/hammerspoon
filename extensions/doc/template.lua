--- === {PATH}.doc ===
---
--- Create doc objects which can be access from within the command line tool or the {TARGET} console for module documentation.
---
--- e.g. `doc = {PATH}.doc.from_json_file(path-to-docs.json)`
---
...
--- {PATH}.doc.from_json_file(jsonfile) -> doc-array
--- Function
--- Builds a doc array construct from the json file provided.  Usually this will be the json file provided with the {TARGET} application, but this json file will only contain the documentation for modules recognized by the Hannerspoon builders and the built in modules and not modules from other sources.  For those, use `{PATH}.doc.from_array` and `{PATH}.doc.from_package_loaded`.
function module.build_from_file(docsfile)
...
--- {PATH}.doc.from_array(array) -> doc-array
--- Function
...
--- {PATH}.doc.from_package_loaded([bool]) -> doc-array
--- Function
