...
local function parseCommand(rule,ielem,icmd,irule,log)
  ...
  elseif r.action==RESTORE then return validateCommand(r,ielem,icmd,irule,log) end
  ...
  if r.action==HIDE or r.action==MINIMIZE or r.action==UNHIDE then
    return validateCommand(r,ielem,icmd,irule,log)
  ...
  validateCommand(r,ielem,icmd,irule,log)
  ...
  while icmd<=#rule and windows[1] do
    ...
    if action==HIDE or action==UNHIDE then
      ...
      if not appbuf[app] or appbuf[app].irule>irule then
        appbuf[app]={hide=action==HIDE,log=log,irule=irule,icmd=icmd}
      end
      ...
      if not buffered or buffered.irule>irule then
        ...
        log.df('pending action "%s" for %s (%d), %d total',action,win:application():name(),win:id(),tlen(winbuf))
      end
    end
